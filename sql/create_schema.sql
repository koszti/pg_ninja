...
(
	...
	REFERENCES  sch_ninja_v2.t_sources(i_id_source)
	ON UPDATE RESTRICT ON DELETE CASCADE
)
;

CREATE TABLE sch_ninja_v2.t_last_replayed
(
	i_id_source			bigserial,
...
CREATE UNIQUE INDEX idx_t_sources_t_source ON sch_ninja_v2.t_sources(t_source);

CREATE TABLE sch_ninja_v2.t_replica_batch
(
...
CREATE UNIQUE INDEX idx_t_replica_batch_binlog_name_position 
    ON sch_ninja_v2.t_replica_batch  (i_id_source,t_binlog_name,i_binlog_position);

CREATE UNIQUE INDEX idx_t_replica_batch_ts_created
	ON sch_ninja_v2.t_replica_batch (i_id_source,ts_created);

CREATE TABLE IF NOT EXISTS sch_ninja_v2.t_log_replica
(
  i_id_event bigserial NOT NULL,
  i_id_batch bigserial NOT NULL,
  v_table_name character varying(100) NOT NULL,
  v_schema_name character varying(100) NOT NULL,
  enm_binlog_event sch_ninja_v2.en_binlog_event NOT NULL,
  t_binlog_name text,
  i_binlog_position integer,
  ...
  CONSTRAINT fk_replica_batch FOREIGN KEY (i_id_batch) 
	REFERENCES  sch_ninja_v2.t_replica_batch (i_id_batch)
	ON UPDATE RESTRICT ON DELETE CASCADE
)
;


CREATE TABLE sch_ninja_v2.t_replica_tables
(
...
CREATE UNIQUE INDEX idx_t_replica_tables_table_schema
	ON sch_ninja_v2.t_replica_tables (i_id_source,v_table_name,v_schema_name);


CREATE TABLE sch_ninja_v2.t_discarded_rows
(
	i_id_row		bigserial,
...
ALTER TABLE sch_ninja_v2.t_replica_batch
	ADD CONSTRAINT fk_t_replica_batch_i_id_source FOREIGN KEY (i_id_source)
	REFERENCES sch_ninja_v2.t_sources (i_id_source)
	ON UPDATE RESTRICT ON DELETE CASCADE
	;

ALTER TABLE sch_ninja_v2.t_replica_tables
...
CREATE TABLE sch_ninja_v2.t_batch_events
(
	i_id_batch	bigint NOT NULL,
...
ALTER TABLE sch_ninja_v2.t_batch_events
...
--FUNCTIONS
CREATE OR REPLACE FUNCTION sch_ninja_v2.fn_refresh_parts() 
RETURNS VOID as 
$BODY$
DECLARE
    t_sql text;
    r_tables record;
BEGIN
    FOR r_tables IN SELECT unnest(v_log_table) as v_log_table FROM sch_ninja_v2.t_sources
    LOOP
        RAISE DEBUG 'CREATING TABLE %', r_tables.v_log_table;
        t_sql:=format('
			CREATE TABLE IF NOT EXISTS sch_ninja_v2.%I
			(
			...
			INHERITS (sch_ninja_v2.t_log_replica)
			;',
...
CREATE OR REPLACE FUNCTION sch_ninja_v2.fn_process_batch(integer,integer)
RETURNS BOOLEAN AS
$BODY$
	DECLARE
	...
	BEGIN
		...
		v_i_id_batch:= (
			SELECT 
				bat.i_id_batch 
			FROM 
				sch_ninja_v2.t_replica_batch bat
				INNER JOIN  sch_ninja_v2.t_batch_events evt
				ON
					...
					(
						...
						FROM 
							(
								...
								FROM 
									sch_ninja_v2.t_log_replica  log
									INNER JOIN sch_ninja_v2.t_replica_tables tab
										ON
		...
		THEN
			DELETE FROM sch_ninja_v2.t_log_replica
			WHERE
    			    i_id_batch=v_i_id_batch
			;
			...
			UPDATE ONLY sch_ninja_v2.t_replica_batch  
			SET 
				b_replayed=True,
			...
			DELETE FROM sch_ninja_v2.t_batch_events
			WHERE
				i_id_batch=v_i_id_batch
			;
		...
		ELSE
			...
			UPDATE sch_ninja_v2.t_batch_events
				SET
